...
def login_user(username, password):
    ...
    for u in users:
        if u["username"] == username and u["password"] == password:
            ...
            # Instead of experimental_rerun, use a dummy state toggle to trigger re-run
...
# ---------------------------
# This allows Home buttons to redirect safely
...
if st.session_state["user"] is None:
    ...
    st.info("Double click on the Login button if it doesn't work the first time.")
    ...
    page = st.sidebar.radio(
        "Navigate",
        ["Home", "Food Logging", "AI Suggestions", "Visualization"],
        index=["Home", "Food Logging", "AI Suggestions", "Visualization"].index(st.session_state["page"]) 
        if st.session_state["page"] in ["Home", "Food Logging", "AI Suggestions", "Visualization"] 
        else 0
    )

    st.sidebar.button("Logout", on_click=lambda: st.session_state.update({"user": None, "page": "Welcome"}))

    # Update session_state page from sidebar selection
