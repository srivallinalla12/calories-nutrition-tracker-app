...
with col1:
    ...
    if nutrients.sum() > 0:
        nutrients.plot(kind="pie", autopct="%1.1f%%", colors=["#FF9999","#66B3FF","#99FF99"], startangle=90, ax=ax1)
        ax1.set_ylabel("")
...
with col2:
    ...
    if cal_data.sum() > 0:
        cal_data.plot(kind="pie", autopct="%1.1f%%", colors=["#FFA07A","#87CEFA"], startangle=90, ax=ax2)
        ax2.set_ylabel("")
...
with col3:
    ...
    if not meal_totals.empty:
        ...
        meal_totals.set_index("MealType")[["Protein","Carbs","Fat"]].plot(
            kind="bar", stacked=True, ax=ax3, color=["#FF9999","#66B3FF","#99FF99"]
        )
...
fig5, ax5 = plt.subplots(figsize=(6,1.2))
macro_props = pd.DataFrame({
    "Protein":[total_protein],
    "Carbs":[total_carbs],
    "Fat":[total_fat]
})
macro_props.plot(kind="barh", stacked=True, color=["#FF9999","#66B3FF","#99FF99"], ax=ax5)
